...
Request Collector имеет приоритет `90`, что означает, что он отображается одним из первых.
...
- Все серверные переменные: `***HIDDEN (PRODUCTION MODE)***`
- Визуальная индикация: красный badge "🔒 PRODUCTION MODE"
- Предупреждающее сообщение перед таблицей

**Философия "Hide All":**  
Вместо поддержки белых/черных списков (которые могут устареть), мы полностью скрываем 
все серверные переменные в production для максимальной безопасности.
