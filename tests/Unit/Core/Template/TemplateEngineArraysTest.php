...
test('can handle undefined array keys gracefully', function () {
    ...
    $templateFile = $this->testTemplateDir . '/undefined_key.twig';
    file_put_contents($templateFile, $templateContent);

    $engine = new TemplateEngine($this->testTemplateDir, $this->testCacheDir);
    $result = $engine->render('undefined_key.twig', [
        'user' => ['name' => 'John']
    ]);
...
test('can render template with numeric array access', function () {
    ...
    $templateFile = $this->testTemplateDir . '/numeric.twig';
    file_put_contents($templateFile, $templateContent);

    $engine = new TemplateEngine($this->testTemplateDir, $this->testCacheDir);
    $result = $engine->render('numeric.twig', [
        'numbers' => [10, 20, 30]
    ]);
...
test('can render template with associative array access', function () {
    ...
    $templateFile = $this->testTemplateDir . '/associative.twig';
    file_put_contents($templateFile, $templateContent);

    $engine = new TemplateEngine($this->testTemplateDir, $this->testCacheDir);
    $result = $engine->render('associative.twig', [
        'person' => [
            'name' => 'Alice',
            'age' => 25,
            'city' => 'London'
