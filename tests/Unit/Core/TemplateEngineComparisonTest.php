...
test('can render template with array element comparison', function () {
    ...
    $templateFile = $this->testTemplateDir . '/array_element_comparison.tpl';
    file_put_contents($templateFile, $templateContent);

    $engine = new TemplateEngine($this->testTemplateDir, $this->testCacheDir);

    $result1 = $engine->render('array_element_comparison.tpl', ['scores' => [85, 70, 90]]);
    expect($result1)->toBe('Excellent!');

    $result2 = $engine->render('array_element_comparison.tpl', ['scores' => [75, 70, 90]]);
    expect($result2)->toBe('');
});
...
test('can handle else branch in conditions', function () {
    ...
    $templateFile = $this->testTemplateDir . '/else_branch.tpl';
    file_put_contents($templateFile, $templateContent);

    $engine = new TemplateEngine($this->testTemplateDir, $this->testCacheDir);

    $result1 = $engine->render('else_branch.tpl', ['age' => 20]);
    expect($result1)->toBe('Adult');

    $result2 = $engine->render('else_branch.tpl', ['age' => 16]);
...
test('can handle negative numbers in comparisons', function () {
    ...
    $templateFile = $this->testTemplateDir . '/negative_numbers.tpl';
    file_put_contents($templateFile, $templateContent);

    $engine = new TemplateEngine($this->testTemplateDir, $this->testCacheDir);

    $result1 = $engine->render('negative_numbers.tpl', ['temperature' => -5]);
    expect($result1)->toBe('Freezing!');

    $result2 = $engine->render('negative_numbers.tpl', ['temperature' => 5]);
    expect($result2)->toBe('');
});

test('can handle zero in comparisons', function () {
    $templateContent = '{% if value == 0 %}Zero detected{% endif %}';
    $templateFile = $this->testTemplateDir . '/zero_comparison.tpl';
    file_put_contents($templateFile, $templateContent);

    $engine = new TemplateEngine($this->testTemplateDir, $this->testCacheDir);

    $result1 = $engine->render('zero_comparison.tpl', ['value' => 0]);
    expect($result1)->toBe('Zero detected');

    $result2 = $engine->render('zero_comparison.tpl', ['value' => 1]);
    expect($result2)->toBe('');
});

test('can handle boolean comparisons', function () {
    $templateContent = '{% if isActive == true %}Active!{% endif %}';
    $templateFile = $this->testTemplateDir . '/boolean_comparison.tpl';
    file_put_contents($templateFile, $templateContent);

    $engine = new TemplateEngine($this->testTemplateDir, $this->testCacheDir);

    $result1 = $engine->render('boolean_comparison.tpl', ['isActive' => true]);
    expect($result1)->toBe('Active!');

    $result2 = $engine->render('boolean_comparison.tpl', ['isActive' => false]);
    expect($result2)->toBe('');
});

test('can handle empty string comparisons', function () {
    $templateContent = '{% if name == "" %}No name{% endif %}';
    $templateFile = $this->testTemplateDir . '/empty_string.tpl';
    file_put_contents($templateFile, $templateContent);

    $engine = new TemplateEngine($this->testTemplateDir, $this->testCacheDir);

    $result1 = $engine->render('empty_string.tpl', ['name' => '']);
    expect($result1)->toBe('No name');

    $result2 = $engine->render('empty_string.tpl', ['name' => 'John']);
    expect($result2)->toBe('');
});

test('can handle type coercion in comparisons', function () {
    $templateContent = '{% if age == "18" %}Match{% endif %}';
    $templateFile = $this->testTemplateDir . '/type_coercion.tpl';
    file_put_contents($templateFile, $templateContent);

    $engine = new TemplateEngine($this->testTemplateDir, $this->testCacheDir);

    // Тест показывает, как движок обрабатывает сравнение числа со строкой
    $result = $engine->render('type_coercion.tpl', ['age' => 18]);
...
test('can handle or operator in comparisons', function () {
    ...
    $templateFile = $this->testTemplateDir . '/or_operator.tpl';
    file_put_contents($templateFile, $templateContent);

    $engine = new TemplateEngine($this->testTemplateDir, $this->testCacheDir);

    $result1 = $engine->render('or_operator.tpl', ['age' => 16]);
    expect($result1)->toBe('Special rate');

    $result2 = $engine->render('or_operator.tpl', ['age' => 70]);
    expect($result2)->toBe('Special rate');

    $result3 = $engine->render('or_operator.tpl', ['age' => 30]);
    expect($result3)->toBe('');
});

test('can handle nested conditions', function () {
    $templateContent = '{% if age >= 18 %}{% if hasLicense == true %}Can drive{% endif %}{% endif %}';
    $templateFile = $this->testTemplateDir . '/nested_conditions.tpl';
    file_put_contents($templateFile, $templateContent);

    $engine = new TemplateEngine($this->testTemplateDir, $this->testCacheDir);

    $result1 = $engine->render('nested_conditions.tpl', ['age' => 20, 'hasLicense' => true]);
    expect($result1)->toBe('Can drive');

    $result2 = $engine->render('nested_conditions.tpl', ['age' => 20, 'hasLicense' => false]);
    expect($result2)->toBe('');

    $result3 = $engine->render('nested_conditions.tpl', ['age' => 16, 'hasLicense' => true]);
    expect($result3)->toBe('');
});

test('can handle float comparisons', function () {
    $templateContent = '{% if price > 99.99 %}Expensive{% endif %}';
    $templateFile = $this->testTemplateDir . '/float_comparison.tpl';
    file_put_contents($templateFile, $templateContent);

    $engine = new TemplateEngine($this->testTemplateDir, $this->testCacheDir);

    $result1 = $engine->render('float_comparison.tpl', ['price' => 150.50]);
    expect($result1)->toBe('Expensive');

    $result2 = $engine->render('float_comparison.tpl', ['price' => 50.00]);
    expect($result2)->toBe('');
});
