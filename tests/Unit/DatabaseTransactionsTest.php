...
it('handles sequential transactions', function (): void {
    // SQLite не поддерживает вложенные транзакции, поэтому тестируем последовательные
    $result1 = $this->db->transaction(function ($db) {
        $db->insert('INSERT INTO accounts (name, balance) VALUES (?, ?)', ['First Account', 100.00]);
        return 'first_success';
    });

    $result2 = $this->db->transaction(function ($db) {
        $db->insert('INSERT INTO accounts (name, balance) VALUES (?, ?)', ['Second Account', 200.00]);
        return 'second_success';
    });

    expect($result1)->toBe('first_success');
    expect($result2)->toBe('second_success');

    // Проверяем, что оба аккаунта созданы
    $firstCount = $this->connection->query('SELECT COUNT(*) FROM accounts WHERE name = "First Account"')->fetchColumn();
    $secondCount = $this->connection->query('SELECT COUNT(*) FROM accounts WHERE name = "Second Account"')->fetchColumn();
...
it('rolls back transaction on exception', function (): void {
    expect(fn() => $this->db->transaction(function ($db) {
        ...
        // Имитируем ошибку в середине транзакции
        throw new Exception('Transaction error');

        return 'success';
    }))->toThrow(Exception::class, 'Transaction error');

    // Проверяем, что аккаунт не создан
    $count = $this->connection->query('SELECT COUNT(*) FROM accounts WHERE name = "Test Account"')->fetchColumn();
    expect($count)->toBe(0);
});
